 ...
 * this class allows it to function as a class entity.
 * <p>
...
public class JTablet {
	...
	public JTablet(boolean fullControl) throws JTabletException {
		if (!library_loaded) throw new JTabletException("Could not load JTablet native library.");
		if (!tabletAvailable()) throw new JTabletException("Tablet not available.");
	 ...
	 * the latest information will be retrieved, rather than the next available information.
	 *
	 * It may be useful to disable this option if you want to retrieve an extended
	 ...
	 * If this doesn't match the return of JTablet.getVersion(), results may
	 * be inaccurate.
	 * @return a String containing version information.
	 * @since 0.9.2
	 */
	 ...
	 *
	 * If poll mode is set to latest, then this will return the last known information
	 * from the tablet, skipping anything since the last poll.
	 *
	 * @see #isPollModeLatest
	 * @see #setPollModeLatest(boolean)
	...
	/**
	 * Retrieves the current pressure of the tablet as an int.  Use <code>getPressureExtent()</code>
	...
	/**
	 * Retrieves the orientation of the tablet.  If this value is negative, the user
	 * has turned their stylus upside-down and is using it as an eraser.
	 *
	...
	private static synchronized native void initializeTablet(boolean fullControl) throws JTabletException;
	private static synchronized native void closeTablet();
	private static synchronized native JTabletCursor pollCursor(boolean pollToLatest) throws JTabletException;
	// Package method
	static native synchronized void getCursorData(JTabletCursor cursor, int type) throws JTabletException;
